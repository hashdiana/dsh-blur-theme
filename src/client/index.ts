...
// Type-only: pulls the settings Context merge (ctx.settingsScope) and the
// settings slot-map merge (settings.general.item).
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.inject(['settingsScope'], (settingsCtx) => {
    const scope = settingsCtx.settingsScope.bind<CardBlurSettings>({ namespace: SETTINGS_NAMESPACE })
