...
import { CARD_BLUR_DEFAULT, CARD_BLUR_FIELD, CARD_BLUR_MAX, CARD_BLUR_MIN, type CardBlurSettings } from '../shared/settings.ts'
...
function readStored(): number {
  try {
    ...
    if (raw === null) return CARD_BLUR_DEFAULT
...
export function createBlurStore(): BlurStore {
  let value = readStored()
  const listeners = new Set<() => void>()
  const getSnapshot = (): BlurStoreSnapshot => ({ value: { [CARD_BLUR_FIELD]: value } })
  return {
    getSnapshot,
