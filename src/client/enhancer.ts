...
const GB_MARKERS = [GB_ROOT, GB_SIDEBAR_COL, GB_SIDEBAR_ROOT, GB_SIDEBAR_SCROLLPORT, GB_SIDEBAR_LIST, GB_CHAT_VIEWPORT, GB_HEADER] as const
...
/** Locate the surfaces the stylesheet targets, from the live DOM only. */
interface Anchors {
  /** The three-column frame root. */
  readonly frame: HTMLElement | null
  /** The sidebar column (first grid child of the frame). */
  readonly sidebarCol: HTMLElement | null
  /** The real sidebar root (past the slot's contents wrapper). */
  readonly sidebarRoot: HTMLElement | null
  /** Parent of the workspace tree list: the fade host. */
  readonly sidebarScrollport: HTMLElement | null
  /** The workspace tree list element (the scroller). */
  readonly sidebarList: HTMLElement | null
  /** The conversation root column (`div[data-phase]`). */
  readonly root: HTMLElement | null
  /** The session header (`<header>`), past the slot's contents wrapper. */
  readonly header: HTMLElement | null
  /** The conversation scroll wrapper (`data-conversation-scroll`). */
  readonly scrollBody: HTMLElement | null
  /** The conversation view area (chat viewport; past the contents wrapper). */
 ...
 */
function skipContentsWrappers(el: Element | null | undefined): Element | null {
  let current = el ?? null
...
function findAnchors(): Anchors {
  const overlay = document.querySelector<HTMLElement>(HOOK_OVERLAY)
  ...
  const sidebarList = sidebarCol?.querySelector<HTMLElement>('[role="tree"]') ?? null
  const sidebarScrollport = sidebarList?.parentElement instanceof HTMLElement ? sidebarList.parentElement : null

  const scrollBody = document.querySelector<HTMLElement>(HOOK_SCROLL_BODY)
  ...
  const root = rootCandidate instanceof HTMLElement
    ...
    ? rootCandidate
    : document.querySelector<HTMLElement>('div[data-phase]')
  const headerEl = skipContentsWrappers(root?.firstElementChild)
  const header = headerEl?.tagName === 'HEADER' ? headerEl as HTMLElement : null
...
export function scrollState(scrollEl: HTMLElement): { top: boolean; bottom: boolean } {
...
function mark(el: HTMLElement, attr: string, tracked: Set<HTMLElement>): void {
  if (!el.hasAttribute(attr)) el.setAttribute(attr, '')
  tracked.add(el)
}

/** Project a scroller's state onto its fade host's presence flags. */
function syncScroll(scrollEl: HTMLElement | null, hostEl: HTMLElement | null): void {
 ...
 */
function hideBuiltinFade(scrollport: HTMLElement, restore: Set<HTMLElement>): void {
 ...
 */
function publishHeaderVar(header: HTMLElement, root: HTMLElement): void {
 ...
 */
function ensureFadeOverlay(attr: string, ownedNodes: Set<HTMLElement>): HTMLElement {
...
function positionFade(fade: HTMLElement, box: { left: number; top: number; width: number; height: number }): void {
...
export function enhance(): () => void {
  ...
  const restoredFades = new Set<HTMLElement>()
  const ownedNodes = new Set<HTMLElement>()
  const observedHeaders = new WeakSet<HTMLElement>()
  let pending = 0
  ...
  const schedule = () => {
    ...
    pending = typeof requestAnimationFrame === 'function'
      ...
      : window.setTimeout(() => { pending = 0; pass() }, 0) as unknown as number
  }
