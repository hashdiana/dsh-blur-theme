/**
 * Shared tsdown preset for UI plugin client bundles, vendored and adapted
 * from the official DeepSeek Harness `packages/client/tsdown.client.ts`
 * (MIT). Emits a closure-factory artifact: the bundle calls
...
/**
 * Shared browser platform modules (mirrors
 * packages/client/web/src/platform.ts in the official repo): the specifiers
 * the shell shares into the frozen module table. A require() the table cannot
 * answer is a guaranteed runtime throw, so the external rule is the table
 * list itself.
 */
 ...
 * PLATFORM_MODULES): the snapshot-store engine (createSnapshotStore/
...
/** Rebase a physical lib-relative source onto a source-relative browser URL. */
 ...
 * any root workspace layout, so the lib half must be restated here — dropping
 ...
 * @param libEntry - node-half entries (tsc-emitted, spelled at the call site).
 * @param options - face handling and lib overrides.
...
function clientConfig(id: string, entry: string): UserConfig {
  return {
    ...
    outputOptions: {
      ...
      // The map is served from /plugins/<scoped-package>/client.js.map.
      // sourcesContent keeps local sources usable without exposing the tree
      // as an HTTP route.
